  ...
  async fetchNewMemeCoins(): Promise<MemeCoin[]> {
    ...
    try {
      ...
      const filteredPairs = data.pairs.filter((pair: any) => {
        const ageInSeconds = Date.now() / 1000 - (pair.pairCreatedAt || 0);
      ...
      for (const pair of filteredPairs.slice(0, 20)) { // Limit to 20 coins
  ...
  private async processCoinData(pair: any): Promise<MemeCoin | null> {
    try {
      ...
      const coin: MemeCoin = {
        ...
        age: Date.now() / 1000 - (pair.pairCreatedAt || 0),
        holders,
        legitScore: 0,
        riskScore: 0,
        rewardScore: 0,
